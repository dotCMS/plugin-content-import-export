	...
	private void importFromContent(
		...
		try {
			...
			List<Contentlet> hits = APILocator.getContentletAPI().search(
				luceneQuery, Integer.MAX_VALUE, 0, "modDate asc", APILocator.getUserAPI().getSystemUser(), false
			);
				...
				try {
					...
					try {
						...
						moveImportedAsset(
							...
							fileName + " - " + UtilMethods.dateToHTMLDate(new Date(), "yyyyMMddHHmmss") + ".cvs.old"
						);
